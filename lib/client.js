window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".wd-deck-add{flex-basis:100%;height:24px;margin-top:2px;border:1px dashed var(--dsw-alias-border-l1);border-radius:7px;background:none;color:var(--dsw-alias-label-tertiary);cursor:pointer;font-size:10px;display:flex;align-items:center;justify-content:center}",
    ...
    function loadState(sessionId) {
      ...
      // 1) 本对话独立布局优先
      const own = read(stateKey(sessionId));
      if (own) return own;
      // 2) 老用户迁移：首次访问该对话时，若存在旧全局布局，迁移为默认布局（不再写回全局）
      if (sessionId) {
        const legacy = read(STATE_KEY);
        if (legacy) {
          try { localStorage.setItem(stateKey(sessionId), JSON.stringify(legacy)); } catch (e) { /* ignore */ }
          return legacy;
        }
      }
      // 3) 全新对话：默认布局
    ...
    function ImageRelayWidget({ api, config, onConfig, width }) {
      ...
      const relay = async (blob, mime, append) => {
        ...
        try {
          ...
          setDesc(finalDesc);
          // 注入输入框：
          // - 自动协作（append=true）：描述拼到用户文本下方，用分隔线隔开——用户正常打字在描述上方，
          //   发送时 DeepSeek 拿到"用户问题 + 完整描述"。
          // - 手动选图（append=false）：直接放描述。
          try {
            if (api && api.inputActions && api.inputActions.setDraft) {
              const sep = "\n\n━━━━━━━━━━━━━━━━━━━━\n📷 已转述图片（供 AI 理解，可忽略）\n" + text.trim() + "\n━━━━━━━━━━━━━━━━━━━━\n";
              // 清掉输入框里上一次的图片描述段（━━ 分隔线区块），再拼新的，避免多图叠加
              const base = append && api.inputDraft ? api.inputDraft.replace(/\n*━━+[^]*?━━+\n*/g, "").trim() : "";
              api.inputActions.setDraft(append ? (base ? base + "\n\n" + sep.trim() : sep.trim()) : finalDesc);
              // 光标放回用户文本末尾（描述上方），用户可继续正常打字
              if (append) {
                window.setTimeout(() => {
                  try {
      ...
      useEffect(() => {
        ...
        const t = window.setTimeout(async () => {
          try {
            ...
            const finalDesc = await relay(blob, mime, true);
      ...
      // 手动选图（现有逻辑）
      const pickRelay = async (file) => { await relay(file, file.type, false); };
    ...
    function WidgetDock({ useProjection, useSessions, useInput, inputActions: propsInputActions, sessionId, executeCommand, openSession }) {
      ...
      // 切换对话时加载该对话独立的卡片布局
      // sidRef：updateConfig 等异步回调写入时用当前生效的 sessionId，避免切对话瞬间写错分区
      ...
      const persist = (next) => { setState(next); saveState(sidRef.current, next); };
      ...
      // 当前草稿文本（自动转述时把描述拼到用户文本下方，保留用户已输入内容）
      const inputDraft = typeof useInput === "function" ? useInput((s) => (s && typeof s.draft === "string" ? s.draft : ""), (a, b) => a === b) : "";
      // inputActions 注入图片转述卡片的 setDraft（写入输入框草稿）
      const api = { useProjection: useProjection, sessionsData: sessionsData, sessionId: sessionId, executeCommand: executeCommand, openSession: openSession, inputImageCount: inputImageCount, inputImageIds: inputImageIds, inputDraft: inputDraft, inputActions: propsInputActions };
