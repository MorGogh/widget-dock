window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".wd-deck{position:fixed;top:92px;z-index:9000;display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));align-content:start;gap:10px;color:var(--dsw-alias-label-primary)}",
    ...
    function loadState() {
      const withDefaults = (items) => {
        ...
        try { shouldRebalance = localStorage.getItem(LAYOUT_VERSION_KEY) !== "2"; } catch (e) { /* ignore */ }
        ...
        try { localStorage.setItem(LAYOUT_VERSION_KEY, "2"); } catch (e) { /* ignore */ }
        return { items: next };
      };
      try {
    ...
    function WidgetDock({ useProjection, sessionId, remote }) {
      ...
      useEffect(() => {
        const measure = () => {
          ...
          const leftStart = 294;
          const leftWidth = Math.max(0, contentLeft - leftStart - 14);
      ...
      const deckStyle = (side) => {
        ...
        return { left: layout[side].x + "px", width: layout[side].width + "px" };
      };
