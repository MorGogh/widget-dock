window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".wd-reason-model-btn{width:100%;height:26px;display:flex;align-items:center;justify-content:space-between;padding:0 9px;border:1px solid var(--dsw-alias-border-l1);border-radius:7px;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);cursor:pointer;font-size:11px}",
      ".wd-reason-model-btn:hover,.wd-reason-model-btn.open{border-color:var(--dsw-alias-state-business-primary)}",
      ...
      // 滑轨（参考官方 slider 视觉：圆角轨道 + 蓝色填充 + tick + 白色圆点）
      ".wd-reason-track{position:relative;height:32px;margin-top:6px;touch-action:none;cursor:pointer;outline:none}",
      ".wd-reason-track:focus-visible .wd-reason-thumb{box-shadow:0 0 0 3px color-mix(in srgb, var(--dsw-alias-state-business-primary) 35%, transparent)}",
      ...
      ".wd-reason-fill{position:absolute;left:-12px;top:4px;height:24px;border-radius:12px;background:var(--dsw-alias-state-business-primary)}",
      ".wd-reason-tick{position:absolute;top:10px;width:12px;height:12px;margin-left:-6px;border-radius:50%;background:var(--dsw-alias-label-tertiary)}",
      ".wd-reason-tick.on{background:var(--dsw-alias-state-business-primary)}",
      ".wd-reason-thumb{position:absolute;top:2.5px;width:27px;height:27px;margin-left:-13.5px;border-radius:50%;background:#fff;box-shadow:0 1px 4px rgba(0,0,0,.25);box-sizing:border-box;transition:left .16s cubic-bezier(.2,.8,.3,1)}",
    ...
    function ReasoningWidget({ api, width }) {
      ...
      return React.createElement("div", { className: "wd-reason-root" },
        React.createElement("div", { className: "wd-balance-amount" },
          React.createElement("span", { className: "wd-balance-val", style: { fontSize: 20 } }, efforts[Math.round(localPos)] ? efforts[Math.round(localPos)].name : (effortLabel || (current ? current.model : "--"))),
