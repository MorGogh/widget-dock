window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const VISION_DEFAULT_MODEL = "qwen-vl-plus";
    const VISION_DEFAULT_URL = "https://api.openai.com/v1/chat/completions";
    ...
    function ImageRelayWidget({ api, config, onConfig, width }) {
      ...
      const [modelDraft, setModelDraft] = useState((config && config.model) || VISION_DEFAULT_MODEL);
      const [urlDraft, setUrlDraft] = useState((config && config.url) || VISION_DEFAULT_URL);
      const fileRef = { current: null };
      const tier = width < 190 ? "S" : (width < 260 ? "M" : (width < 350 ? "L" : "XL"));
      const hasImages = api.inputImageCount > 0;

      const relay = async (file) => {
        setBusy(true); setError(null); setDesc(null);
        try {
          const key = getVisionKey(config);
          if (!key) throw new Error("未配置视觉模型 KEY");
          const model = (config && config.model) || VISION_DEFAULT_MODEL;
          const url = (config && config.url) || VISION_DEFAULT_URL;
          ...
          const res = await fetch(url, {
            ...
            body: JSON.stringify({
              ...
              messages: [{ role: "user", content: [
                ...
                { type: "text", text: "请详细描述这张图片的内容，包括主要元素、文字、布局和细节，供一个无法查看图片的纯文本助手理解。" }
              ] }]
            })
          });
          if (!res.ok) {
          ...
          const finalDesc = "[图片转述] " + text.trim();
          setDesc(finalDesc);
      ...
      return React.createElement("div", null,
        ...
        React.createElement("input", {
          ...
          accept: "image/*",
          style: { display: "none" },
          onChange: (e) => { const f = e.target.files && e.target.files[0]; if (f) relay(f); e.target.value = ""; }
        }),
        ...
        editing ? React.createElement("div", { className: "wd-edit" },
          ...
          React.createElement("div", { className: "wd-edit-row" },
            ...
            getVisionKey(config) ? "模型: " + ((config && config.model) || VISION_DEFAULT_MODEL) : "未配置视觉模型 KEY"
          ) : null
      );
    }
    ...
    function WidgetDock({ useProjection, useSessions, useInput, inputActions: propsInputActions, sessionId, executeCommand, openSession }) {
      ...
      const api = { useProjection: useProjection, sessionsData: sessionsData, sessionId: sessionId, executeCommand: executeCommand, openSession: openSession, inputImageCount: inputImageCount, inputActions: propsInputActions };
