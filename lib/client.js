window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SessionTimeMachineWidget({ api, width }) {
      const [notice, setNotice] = useState(null);
      // 数据来自 WidgetDock 顶层的 useSessions（sessionsData），此处不做 hook 调用
      ...
      return React.createElement("div", null,
        ...
        ids && ids.length ? React.createElement("div", { className: "wd-balance-detail", style: { display: "block", marginTop: 4 } },
          shown.map((id) => {
            ...
            const label = s && (s.displayTitle || s.title || String(id).slice(0, 8));
    ...
    function WorkspaceCompassWidget({ api, width }) {
      ...
      const cwd = current && sessions.byId[current] ? sessions.byId[current].cwd : null;
    ...
    function WidgetDock({ useProjection, useSessions, sessionId, executeCommand, openSession }) {
      ...
      // 稳定调用 useSessions（typeof 判断恒定，hook 调用顺序稳定）：拿会话列表数据
      const sessionsData = typeof useSessions === "function" ? useSessions((s) => s) : null;
      const persist = (next) => { setState(next); saveState(next); };
      const items = state.items.filter((item) => WIDGETS[item.id]);
      const api = { useProjection: useProjection, useSessions: useSessions, sessionsData: sessionsData, sessionId: sessionId, executeCommand: executeCommand, openSession: openSession };
