window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      try {
        ...
        for (let i = 0; i < all.length; i++) {
          ...
          if (cs.getPropertyValue("--dsh-chat-content-width")) {
            ...
            if (r.width > 300 && r.height > 200 && r.right < vw - 40) return el;
          }
        }
      } catch (e) { /* ignore */ }
      return null;
    ...
    const css = [
      ...
      // 工作台：固定宽度、固定顺序，避免悬浮卡片散落在内容区域。
      ".wd-workbench{position:fixed;right:18px;top:92px;width:268px;z-index:9000;border:1px solid var(--dsw-alias-border-l1);border-radius:14px;background:var(--dsw-alias-bg-base);box-shadow:0 14px 42px rgba(0,0,0,.2);color:var(--dsw-alias-label-primary)}",
      ".wd-workbench-head{display:flex;align-items:center;justify-content:space-between;padding:12px 12px 9px}",
      ".wd-workbench-title{font-size:13px;font-weight:600}",
      ".wd-workbench-actions{display:flex;align-items:center;gap:3px}",
      ".wd-workbench-subtitle{padding:0 12px 10px;font-size:10px;color:var(--dsw-alias-label-tertiary)}",
      ".wd-workbench-card{margin:0 10px 9px;border:1px solid var(--dsw-alias-border-l1);border-radius:10px;overflow:hidden}",
      ...
      ".wd-carousel{position:relative;overflow:hidden;touch-action:pan-y}",
      ".wd-carousel-stage{min-height:170px}",
      ".wd-carousel-nav{display:flex;align-items:center;justify-content:space-between;padding:0 10px 10px;gap:8px}",
      ".wd-carousel-count{font-size:10px;color:var(--dsw-alias-label-tertiary);flex:1;text-align:center}",
      ".wd-carousel-btn{width:28px;height:26px;border:1px solid var(--dsw-alias-border-l1);border-radius:7px;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-secondary);cursor:pointer;font-size:16px;line-height:1}",
      ".wd-carousel-btn:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}",
    ...
    function loadState() {
      const withDefaults = (items) => {
        ...
        DEFAULT_WIDGET_IDS.forEach((id) => {
          if (!next.some((item) => item && item.id === id)) next.push({ id: id, config: {} });
        });
        return { items: next };
      };
      try {
    ...
    function WorkbenchCard({ item, index, api, onRemove, onConfig }) {
      const widget = WIDGETS[item.id];
      const Content = widget.render;
      return React.createElement("section", { className: "wd-workbench-card" },
        React.createElement("div", { className: "wd-workbench-card-head" },
          React.createElement("span", null, widget.name),
          React.createElement("button", { className: "wd-btn", title: "从工作台移除", onClick: () => onRemove(index) }, "✕")
        ),
        React.createElement("div", { className: "wd-body" },
          React.createElement(ErrorBoundary, { widgetName: widget.name },
            React.createElement(Content, { api: api, config: item.config || {}, onConfig: (config) => onConfig(index, config), width: 246 })
          )
        )
      );
    }

    function WorkbenchCarousel({ items, api, onRemove, onConfig }) {
      ...
      const start = (event) => { pointerStart.current = { x: event.clientX, y: event.clientY }; };
      const end = (event) => {
        ...
        if (Math.abs(dx) > 36 && Math.abs(dx) > Math.abs(dy)) dx < 0 ? next() : previous();
      };
      if (!current) return null;
      return React.createElement("div", { className: "wd-carousel", onPointerDown: start, onPointerUp: end },
        React.createElement("div", { className: "wd-carousel-stage" },
          React.createElement(WorkbenchCard, { item: current, index: safeIndex, api: api, onRemove: onRemove, onConfig: onConfig })
        ),
        items.length > 1 ? React.createElement("div", { className: "wd-carousel-nav" },
          React.createElement("button", { className: "wd-carousel-btn", title: "上一项", onClick: previous }, "‹"),
          React.createElement("span", { className: "wd-carousel-count" }, (safeIndex + 1) + " / " + items.length + " · 左右拖动切换"),
          React.createElement("button", { className: "wd-carousel-btn", title: "下一项", onClick: next }, "›")
        ) : null
      );
    }
    ...
    function WidgetDock({ useProjection, sessionId, remote }) {
      ...
      const toggle = (id) => {
        ...
        if (index >= 0) {
          ...
          persist({ items: items.concat([{ id: id, config: {} }]) });
        }
      };
      const remove = (index) => {
        const next = { items: items.slice() };
      ...
      return React.createElement(React.Fragment, null,
        !open ? React.createElement("aside", { className: "wd-status", "aria-label": "工作状态" },
          React.createElement("div", { className: "wd-status-title" }, "工作状态"),
          React.createElement("div", { className: "wd-status-row" }, React.createElement("span", { className: "wd-status-label" }, "上下文"), React.createElement("span", { className: "wd-status-value" }, tokenTotal == null ? "等待消息" : tokenTotal.toLocaleString())),
          React.createElement("div", { className: "wd-status-row" }, React.createElement("span", { className: "wd-status-label" }, "目标"), React.createElement("span", { className: "wd-status-value" }, goal ? (goal.phase || "进行中") : "未设定")),
          React.createElement("div", { className: "wd-status-row" }, React.createElement("span", { className: "wd-status-label" }, "回合"), React.createElement("span", { className: "wd-status-value" }, turnCount == null ? "—" : turnCount)),
          React.createElement("button", { className: "wd-status-open", onClick: () => setMinimized(false) }, "展开工作台")
        ) : React.createElement("aside", { className: "wd-workbench", "aria-label": "工作台" },
          React.createElement("div", { className: "wd-workbench-head" },
            React.createElement("span", { className: "wd-workbench-title" }, "工作台"),
            React.createElement("div", { className: "wd-workbench-actions" },
              React.createElement("button", { className: "wd-btn", title: pickerOpen ? "返回工作台" : "定制工作台", onClick: () => setPickerOpen(!pickerOpen) }, pickerOpen ? "‹" : "☷"),
              React.createElement("button", { className: "wd-btn", title: "最小化工作台", onClick: () => { setPickerOpen(false); setMinimized(true); } }, "—")
            )
          ),
          pickerOpen ? React.createElement(WidgetPicker, { selected: items.map((item) => item.id), onToggle: toggle }) : React.createElement(React.Fragment, null,
            React.createElement("div", { className: "wd-workbench-subtitle" }, "重要信息在这里，其余内容留给对话。"),
            items.length ? React.createElement(WorkbenchCarousel, { items: items, api: api, onRemove: remove, onConfig: updateConfig }) : React.createElement("div", { className: "wd-empty-workbench" }, "还没有固定的信息。\n点击右上角的定制按钮选择项目。")
          )
        )
      );
    }
