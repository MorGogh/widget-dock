window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 9c) 推理强度：读取会话模型目录（modelDirectories 服务），滑轨切换 reasoningEffort
    function ReasoningWidget({ api, width }) {
      ...
      const effortLabel = (() => {
        ...
        if (effectiveEffort !== undefined) {
          ...
          return found ? found.name : String(effectiveEffort);
        }
        return efforts.length > 0 ? efforts[Math.floor(efforts.length / 2)].name : undefined;
      })();
      ...
      return React.createElement("div", null,
        React.createElement("div", { className: "wd-balance-amount" },
          React.createElement("span", { className: "wd-balance-val", style: { fontSize: 20 } }, effortLabel || (current ? current.model : "--")),
          React.createElement("span", { className: "wd-balance-cur" }, "推理强度")
        ),
        tier !== "S" ? React.createElement("div", { className: "wd-balance-detail" },
          React.createElement("span", null, current ? current.model : "未加载"),
          React.createElement("span", null, busy ? "切换中…" : "")
        ) : null,
        // 档位滑轨：每个 effort 一个圆点，当前档位高亮
        efforts.length > 1 ? React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 4, marginTop: 8, flexWrap: "wrap" } },
          efforts.map((level, i) => React.createElement("button", {
            key: level.id,
            ...
            style: {
              ...
              padding: "0 8px",
              borderRadius: 11,
              border: "1px solid " + (i === resolvedIndex ? "var(--dsw-alias-state-business-primary)" : "var(--dsw-alias-border-l1)"),
              background: i === resolvedIndex ? "var(--dsw-alias-interactive-bg-hover)" : "var(--dsw-alias-bg-base)",
              color: i === resolvedIndex ? "var(--dsw-alias-state-business-primary)" : "var(--dsw-alias-label-secondary)",
              fontSize: 10,
        ...
        error ? React.createElement("div", { className: "wd-err" }, error) : null,
        tier === "XL" ? React.createElement("div", { className: "wd-empty", style: { marginTop: 4 } }, "强度越高思考越久，费用也越高") : null
      );
    }
