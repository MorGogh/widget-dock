window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".wd-picker-drag{flex:none;width:18px;height:26px;display:flex;align-items:center;justify-content:center;color:var(--dsw-alias-label-tertiary);cursor:grab;font-size:12px;border-radius:5px}",
      ...
      // 拖拽添加时侧栏幽灵化：透明 + 穿透点击，让 drop 落到 deck；dragend 恢复
      ".wd-picker-ghost{opacity:.25;pointer-events:none;transition:opacity .15s}",
    ...
    function ImageRelayWidget({ api, config, onConfig, width }) {
      ...
      const writeDescBlock = (text, isNewBatch) => {
        ...
        try {
          const base = isNewBatch ? stripDescBlock(inputDraft) : inputDraft;
      ...
      useEffect(() => {
        ...
        const t = later(async () => {
          try {
            // 从 DOM 抓输入框附件区的图片 blob URL（composer 附件预览 <img>，按添加顺序渲染）
    ...
    function WidgetDock({ useProjection, useSessions, useInput, inputActions: propsInputActions, sessionId, executeCommand, openSession }) {
      ...
      return React.createElement(React.Fragment, null,
        ...
        pickerOpen ? React.createElement(WidgetPicker, {
          ...
          onDragAddStart: () => setPickerGhost(true),
          onDragAddEnd: () => setPickerGhost(false),
          ghost: pickerGhost
        }) : null
      );
    }
