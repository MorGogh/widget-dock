window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".wd-deck-add{display:inline-flex;align-items:center;gap:4px;height:24px;padding:0 8px;border:1px solid var(--dsw-alias-border-l1);border-radius:7px;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-state-business-primary);cursor:pointer;font-size:11px;font-weight:600}",
      ".wd-deck-add:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-state-business-primary)}",
      ...
      ".wd-arrange-tip{grid-column:1/-1;display:flex;align-items:center;gap:6px;margin:-1px 0 1px;padding:7px 8px;border-radius:8px;background:var(--dsw-alias-interactive-bg-hover);font-size:10px;color:var(--dsw-alias-label-secondary)}",
      ...
      ".wd-picker{grid-column:1/-1;padding:0 2px 6px}",
    ...
    function WidgetPicker({ items, onToggle, onMove }) {
      return React.createElement("div", { className: "wd-picker" },
        React.createElement("div", { className: "wd-picker-note" }, "点击添加；已显示的卡片可直接切换到左侧或右侧"),
        ...
        return React.createElement("aside", {
          ...
          React.createElement("div", { className: "wd-deck-title" }, title,
            side === "right" ? React.createElement("div", { className: "wd-deck-actions" },
              React.createElement("button", { className: "wd-deck-add", title: pickerOpen ? "收起添加面板" : "添加或调整面板位置", onClick: () => { setEditing(false); setPickerOpen(!pickerOpen); } }, pickerOpen ? "收起" : "+ 添加面板"),
              React.createElement("button", { className: "wd-deck-arrange" + (editing ? " active" : ""), title: editing ? "完成排序" : "拖动调整卡片排序", onClick: () => { setPickerOpen(false); setEditing(!editing); } }, editing ? "完成" : "排序"),
              React.createElement("button", { className: "wd-btn", title: "最小化工作台", onClick: () => { setPickerOpen(false); setEditing(false); setMinimized(true); } }, "—")
            ) : null
          ),
          editing ? React.createElement("div", { className: "wd-arrange-tip" }, "拖动卡片可排序；拖到另一侧即可切换位置") : null,
          pickerOpen && side === "right" ? React.createElement(WidgetPicker, { items: items, onToggle: toggle, onMove: moveToSide }) : deck.length ? deck.map((item, index) => React.createElement(WorkbenchCard, {
            key: item.id,
            item: item,
            index: index,
            api: api,
